...
interface MoveStrategy {
    int[] getMove(Board board, Scanner sc);
}
class HumanMoveStrategy implements MoveStrategy {

    @Override
    public int[] getMove(Board board, Scanner sc) {
...
class Player {
    ...
    public int[] makeMove(Board board, Scanner sc) {
        return moveStrategy.getMove(board, sc);
    }
}

class PlayerFactory {
    public static Player createPlayer(String name, char symbol, String type) {

        if(type.equalsIgnoreCase("AI")) {
            return new Player(name, symbol, new AIMoveStrategy());
        } else {
            return new Player(name, symbol, new HumanMoveStrategy());
        }
    }
}
...
class Game {
    ...
    public void start() {
        while (true) {
            ...
            int[] move = player.makeMove(board, sc);
            int row = move[0];
            int col = move[1];
...
public class TicTacToeAI {
    ...
    public static void main(String args[]) {
        System.out.println();
        Player p1 = PlayerFactory.createPlayer("Aditya", 'X', "Human");;
        Player p2 = PlayerFactory.createPlayer("Bot", 'O', "AI");
